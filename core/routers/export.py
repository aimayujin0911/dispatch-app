...
from models import Vehicle, Driver, Shipment, Dispatch, Client, PartnerCompany, Attendance, AccountEntry, User
...
@router.get("/vehicles")
def export_vehicles(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    vehicles = db.query(Vehicle).filter(Vehicle.tenant_id == current_user.tenant_id).all()
...
@router.get("/drivers")
def export_drivers(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    drivers = db.query(Driver).filter(Driver.tenant_id == current_user.tenant_id).all()
...
def export_shipments(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    shipments = db.query(Shipment).filter(Shipment.tenant_id == current_user.tenant_id).all()
...
def export_dispatches(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    dispatches = db.query(Dispatch).filter(Dispatch.tenant_id == current_user.tenant_id).all()
    rows = []
    for d in dispatches:
...
@router.get("/partners")
def export_partners(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    partners = db.query(PartnerCompany).filter(PartnerCompany.tenant_id == current_user.tenant_id).all()
...
def export_attendance(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    driver_ids = [d.id for d in db.query(Driver.id).filter(Driver.tenant_id == current_user.tenant_id).all()]
    records = db.query(Attendance).filter(Attendance.driver_id.in_(driver_ids)).order_by(Attendance.date).all()
    rows = []
    for a in records:
...
def export_accounting(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    vehicle_ids = [v.id for v in db.query(Vehicle.id).filter(Vehicle.tenant_id == current_user.tenant_id).all()]
    shipment_ids = [s.id for s in db.query(Shipment.id).filter(Shipment.tenant_id == current_user.tenant_id).all()]
    from sqlalchemy import or_
    entries = db.query(AccountEntry).filter(
        or_(
    ...
    ).order_by(AccountEntry.date).all()
